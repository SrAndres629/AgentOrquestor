...
class AdvancedSkillGenerator:
    ...
    async def step_back_analysis(self, objective: str):
        telemetry.info(f"🫧 [GEN_ Step-Back] Analizando redundancia para: {objective}")
        return False

    async def spawn_expert_skill(self, name, description, logic):
        telemetry.info(f"🔎 [GEN] Forjando Skill Avanzada: {name}")
        skill_dir = os.path.join("/home/jorand/Escritorio/Biblioteca MCP/AgentOrquestor/agents/skills", name)
        os.makedirs(skill_dir, exist_ok=True)
        with open(os.path.join(skill_dir, "executor.py"), "w", encoding="utf-8") as f:
            f.write(logic)
